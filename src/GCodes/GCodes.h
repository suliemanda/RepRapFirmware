...
private:
	...
	bool DoExtrusionOnly(GCodeBuffer& gb,
                                     ...
                                     float feed_mm_s,
                                     bool drivesRelative,
                                     bool volumetricExtrusion,const StringRef& reply) THROWS(GCodeException) SPEED_CRITICAL; // Execute an extrusion-only move with explicit parameters
	...
	GCodeResult StraightProbexyze(GCodeBuffer& gb, const StringRef& reply,int8_t cmd_type, float X_t=SILLY_Z_VALUE,float Y_t=SILLY_Z_VALUE,float Z_t=SILLY_Z_VALUE,float E=SILLY_Z_VALUE,float Feed=-1) THROWS(GCodeException);	// do g38.[2-5] with explicit parameters
	...
	GCodeResult HandleG555(const StringRef& reply,float W=-1.0,float P=-1.0,float E=-1.0,float A=-1.0) THROWS(GCodeException);	// Deal with a G555 with explicit parameters
	...
	void LoadFeedrateFromVal(GCodeBuffer& gb,float val, MovementState& ms, bool axesMoving) THROWS(GCodeException);
